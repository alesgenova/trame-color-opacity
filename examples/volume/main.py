...
from trame_color_opacity import widgets as colormap
...
class VolumeApp(TrameApp):
    def __init__(self, image_data, server=None):
        ...
        self.state.background_shape = "histograms"
        self.state.background_opacity = False
        self.state.colormap_name = "RBG"
        self.state.show_histograms = False
        ...
        with SinglePageLayout(self.server) as layout:
            ...
            with layout.content as c:
                with html.Div(style="display: flex; gap: 1rem; padding: 1rem;"):
                    ...
                    vuetify.VSelect(v_model=("background_shape",), items=("background_shape_options", SHAPER_OPTIONS), label="Background Shape", density="compact")
                    vuetify.VCheckbox(v_model=("background_opacity",), label="Background Opacity")
                    vuetify.VCheckbox(v_model=("show_histograms",), label="Show Histograms")

                colormap.ColorOpacityEditor(
                    style="width: 100%; height: 24rem; padding: 0.5rem;",
