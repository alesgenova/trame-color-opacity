...
const adaptedColorNodes = computed(() =>
  props.colorNodes.map((el) => new MapNodeAdapter(el)),
);
const adaptedOpacityNodes = computed(() =>
  props.opacityNodes.map((el) => new MapNodeAdapter(el)),
);
...
const colorOpacityNodes = computed(() => {
  ...
  while (
    i < adaptedColorNodes.value.length &&
    j < adaptedOpacityNodes.value.length
  ) {
