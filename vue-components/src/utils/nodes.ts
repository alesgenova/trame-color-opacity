...
export function isOpacityNode(
  node: MapNode<unknown> | undefined,
): node is OpacityNode {
...
export function isOpacityNodes(
  nodes: MapNode<unknown>[],
): nodes is OpacityNode[] {
...
export function unscaleNodesX<T>(
  nodes: MapNode<T>[],
  range: Vector2D,
): MapNode<T>[] {
  const span = Math.abs(range[1] - range[0]);
